...
Due to platform limitations enforced by Google, it's not possible to login via the Open Grind app itself. Read the technical in-depth explanation below. **For Android specifically, Open Grind supports signing in with Google via the companion app or manual token paste.**
...
This section answers some questions on why the decision to release a separate companion app for Android was made. It's intended for technical experts who have some knowledge with mobile development and familiar with OAuth protocol.
