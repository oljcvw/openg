<script lang="ts">
	...
	let {
		...
		thumbAriaValueText?: (value: number, index: number) => string | undefined;
	} = $props();
</script>
	...
	{#snippet children({ thumbItems })}
		...
		{#each thumbItems as thumb (thumb.index)}
			<SliderPrimitive.Thumb
				...
				index={thumb.index}
				aria-label={thumbAriaLabels[thumb.index] ?? thumbLabels[thumb.index]}
				aria-valuetext={thumbAriaValueText?.(thumb.value, thumb.index)}
