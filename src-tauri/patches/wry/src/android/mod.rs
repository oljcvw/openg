...
macro_rules! define_static_handlers {
  ...
  ($($var:ident = $type_name:ident { $($fields:ident:$types:ty),+ $(,)? });+ $(;)?) => {
    $(
    static $var: Lazy<Mutex<HashMap<WebviewId, Arc<$type_name>>>> = Lazy::new(||Mutex::new(HashMap::new()));
...
pub struct UnsafeEvalCallback {
  pub handler: Box<dyn Fn(String) + Send + 'static>,
}
unsafe impl Send for UnsafeEvalCallback {}
unsafe impl Sync for UnsafeEvalCallback {}
type EvalCallback = Box<dyn Fn(String) + Send + 'static>;

pub static EVAL_ID_GENERATOR: Counter = Counter::new();
pub static EVAL_CALLBACKS: OnceCell<Mutex<HashMap<i32, Arc<UnsafeEvalCallback>>>> = OnceCell::new();
...
impl InnerWebView {
  ...
  pub fn new(
    ...
    REQUEST_HANDLER
      ...
      .insert(
        id.clone(),
        Arc::new(UnsafeRequestHandler::new(Box::new(
    ...
    if let Some(i) = ipc_handler {
      IPC
        .lock()
        .unwrap()
        .insert(id.clone(), Arc::new(UnsafeIpc::new(Box::new(i))));
    }
    ...
    if let Some(i) = attributes.document_title_changed_handler {
      TITLE_CHANGE_HANDLER
        .lock()
        .unwrap()
        .insert(id.clone(), Arc::new(UnsafeTitleHandler::new(i)));
    }
    ...
    if let Some(i) = attributes.navigation_handler {
      URL_LOADING_OVERRIDE
        .lock()
        .unwrap()
        .insert(id.clone(), Arc::new(UnsafeUrlLoadingOverride::new(i)));
    }
    ...
    if let Some(h) = attributes.on_page_load_handler {
      ON_LOAD_HANDLER
        .lock()
        .unwrap()
        .insert(id.clone(), Arc::new(UnsafeOnPageLoadHandler::new(h)));
    }
